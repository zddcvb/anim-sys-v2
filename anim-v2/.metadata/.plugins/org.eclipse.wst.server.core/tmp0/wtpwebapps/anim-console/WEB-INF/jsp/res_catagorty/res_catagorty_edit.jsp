...
<body>
	...
	<div>
		<form
			action="${pageContext.request.contextPath }/res_catagorty/edit_res_catagorty"
			method="post" class="form-horizontal " role="form">
			<input type="hidden" name="pageNum" value="${pageNum }"> <input
				type="hidden" name="resCatagortyId"
				value="${animResCatagorty.resCatagortyId }"> <input
			...
			<div class="form-group">
				<label for="name" class="col-sm-2 control-label"> 素材分类名称</label> <input
					...
					value="${animResCatagorty.resCatagortyName }">
			</div>

			<div class="form-group">
				<label for="parent" class="col-sm-2 control-label"> 所属分类</label> <select
					name="parentId">
					<option value="${animResCatagorty.parentId }">${animResCatagorty.parent.resCatagortyName }</option>
					<c:forEach items="${parents }" var="parent">
						<c:if test="${animResCatagorty.parentId!=parent.resCatagortyId }">
							<option value="${parent.resCatagortyId }">${parent.resCatagortyName }</option>
						</c:if>
					</c:forEach>
				</select>
			</div>
